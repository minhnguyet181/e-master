...
async function saveAIRecommendation(userId, aiPlan) {
  ...
  // Update user with AI recommendation
 ...
 * Hỗ trợ các format: "Band 5.0", "Band 5", "5.0", "5"
 * 
 * @param {string} bandString - Band string (ví dụ: "Band 5.0", "5", "Band 6.5")
 * @returns {number|null} - Band number hoặc null nếu không parse được
 */
...
/**
 * Normalize band string để so sánh
 * Chuyển các format khác nhau về cùng format để so sánh
 * 
 * @param {string} bandString - Band string
 * @returns {number|null} - Band number normalized
 */
